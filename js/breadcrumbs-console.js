...
const consoleId = getQueryParam('id');
if (consoleId) {
  dbc.collection('consoles').doc(consoleId).get().then(doc => {
    if (doc.exists) {
      const console = doc.data();
      updateBreadcrumb(console.name);
    } else {
...
function updateBreadcrumb(consoleName) {
  const breadcrumb = document.querySelector('.breadcrumb ol');
  if (!breadcrumb) return;

  // Ajoute le jeu en dernier élément (sans lien)
  const li = document.createElement('li');
  li.textContent = consoleName;
